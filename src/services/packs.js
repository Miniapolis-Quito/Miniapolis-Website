...
export function transferTickets(sourcePackId, { quantity, recipient, note = null, idempotencyKey = null, actor, ip = null, userAgent = null }) {
  ...
  if (correoDisponible() && outcome.recipient.email) {
    ...
    const bloquesHtml = [
      ...
      ...(outcome.note
        ? [`<blockquote style="margin:0 0 16px;padding:8px 16px;border-left:4px solid #7ed500;background:#f9f9f9;font-style:italic">"${escaparHtml(outcome.note)}"</blockquote>`]
        : []),
