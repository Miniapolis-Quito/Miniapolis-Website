...
const TODOS = [
  // Galaxy Fold cerrado: el teléfono más estrecho que circula.
  [280, 653],
  [320, 640], [360, 740], [375, 667], [390, 844], [414, 896], [430, 932],
  [600, 960], [768, 1024], [834, 1194], [1024, 768], [1280, 800], [1440, 900], [1920, 1080], [2560, 1440],
  // Teléfono girado: poca altura, que es donde se amontonan las cabeceras fijas.
  [844, 390],
];
// `ANCHOS=280x653,2560x1440` revisa solo esos formatos: para repasar un
// arreglo sin esperar la vuelta completa.
const ANCHOS = process.env.ANCHOS
  ? process.env.ANCHOS.split(',').map((f) => f.split('x').map(Number))
  : TODOS;
...
for (const movimiento of ['no-preference', 'reduce']) {
  ...
  for (const [ancho, alto] of ANCHOS) {
    ...
    if (!hayRiel) continue;
    // El motor mide la recta al arrancar: antes, su altura aún no incluye el
    // tramo del riel y el «final» quedaría antes de que empiece a moverse.
    await riel.waitForFunction(() => document.documentElement.classList.contains('portada-listos'), null, { timeout: 15000 }).catch(() => {});
